...
# Create a 1920x1080 horizontal image
img_path = Path("horiz_image.jpg")
subprocess.run(["ffmpeg", "-y", "-f", "lavfi", "-i", "color=c=red:s=1920x1080", "-frames:v", "1", str(img_path)])

# Run the B-roll generation
broll_video_path = Path("test_horiz_broll.mp4")
cmd = [
...
if res.returncode != 0:
    ...
    print("SUCCESS!")
    print("Check test_horiz_broll.mp4 for stretching.")
