...
class GiphyClient:
    ...
    def search_gifs(self, query: str, limit: int = 5) -> list[BrollAsset]:
        """Search Giphy and return them as BrollAsset objects."""
        assets = []
        ...
        try:
            ...
            url = f"https://api.giphy.com/v1/gifs/search?api_key={giphy_api_key}&q={urllib.parse.quote(clean_query)}&limit={limit}&rating=pg-13"
            ...
            for gif_idx, gif_data in enumerate(results):
                ...
                if mp4_url:
                    asset = BrollAsset(
                        id=f"giphy_{gif_data.get('id', str(gif_idx))}",
                        url=mp4_url,
                        source="giphy"
                    )
                    assets.append(asset)
